...
SimulateWorld <- function(){
  ...
  if (cor(lon@data@values,depth@data@values)>0.6) stop('lon~depth correlation >0.6') 
  ...
  for (y in years){
    ...
    #Generate SeaIce Covariate
    high_years <- c(1,2,3,8,9,10,11) #Decide which years for each sea ice mode
    medium_years <- c(4,5,6,7,12,13,18,19,20)
    low_years <- c(14,15,16,17)
    if (y %in% high_years){
      spatiotemp <- raster("SeaIceShape_High.png")
    }
    if (y %in% medium_years){
      spatiotemp <- raster("SeaIceShape_Medium.png")
    }
    if (y %in% low_years){
      spatiotemp <- raster("SeaIceShape_Low.png")
    }
    ex <- extent(0.5,20.5,0.5,20.5)
    ...
    #----Combing all habitat suitability rasters----
  ...
  #----Generate presence and abundance as a function of habitat suitability----
  grid$pres <- ifelse(grid$suitability<=0.4,0, ifelse(grid$suitability>=0.6,1,rbinom(nrow(grid),1,prob=grid$suitability)))
  grid$catch <- ifelse(grid$pres==1,rlnorm(1,2,0.1)*grid$suitability,0)
